...
{
    /*
    The implementation of these handlers(UsbEvents) are going to be inside the framework.c, 
    but these functions will be called inside the driver layer.
    */
...
#define USB_CLASS_CDC_COMM 0x02 /**<\brief CDC communication class.*/
...
#define USB_CLASS_CDC_DATA 0x0A /**<\brief CDC data class.*/
...
typedef struct {
	...
	uint8_t  bmAttributes; /**<\brief Configuration attributes: Self Powered and Remote Wake up. bit7 = 1: Bus powered, bit6=1: self-powered, bit5=1: remote-wakeup*/
...
typedef struct {
	...
	uint16_t bcdCDC; 
} __attribute__((__packed__)) UsbHeaderFunctionalDescriptor;
...
} __attribute__((__packed__)) UsbCallManagementFunctionalDescriptor;
...
typedef struct {
	...
	uint8_t  bDescriptorSubtype; 
	uint8_t  bmCapabilities; 
} __attribute__((__packed__)) UsbACMFunctionalDescriptor;
